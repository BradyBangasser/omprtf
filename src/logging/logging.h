#ifndef OMPRTF_LOGGING_H
#define OMPRTF_LOGGING_H
...
#define ERRORF(f, ...)                                                         \
  fprintf(stderr, ERROR_COLOR "[ERROR]   " f COLOR_RESET, __VA_ARGS__) &&      \
      ++nerrs;
#define ERROR(f)                                                               \
  fprintf(stderr, ERROR_COLOR "[ERROR]   " f COLOR_RESET) && ++nerrs;
#define WARNF(f, ...)                                                          \
  fprintf(stderr, WARNING_COLOR "[WARNING] " f COLOR_RESET, __VA_ARGS__) &&    \
      ++nwarns
#define WARN(f)                                                                \
  fprintf(stderr, WARNING_COLOR "[WARNING] " f COLOR_RESET) && ++nwarns
...
#define SUCCESSF(f, ...)                                                       \
  printf(SUCCESS_COLOR "[SUCCESS] " f COLOR_RESET, __VA_ARGS__)

#ifdef M_DEBUG
#define DEBUG(f) printf(DEBUG_COLOR "[DEBUG]   " f COLOR_RESET)
#define DEBUGF(f, ...)                                                         \
  printf(DEBUG_COLOR "[DEBUG]   " f COLOR_RESET, __VA_ARGS__)
...
static inline void start_time() { clock_gettime(TIMING_CLOCK, &ts); }
...
static inline void stop_time_print_data() {
  ...
  uint32_t hours = tsec / 3600, minutes = (tsec % 3600) / 60 % 60;
  tsec %= 60;
